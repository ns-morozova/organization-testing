...
var Widget = /*#__PURE__*/function () {
  ...
  return _createClass(Widget, [{
    ...
    key: "checkLuhn",
    value: function checkLuhn(ccn) {
      var ccnS = ccn.replace(/ /g, '').toString();
      var sum = 0;
      var parity = ccnS.length % 2;
      for (var i = 0; i < ccnS.length; i += 1) {
        var digit = Number(ccnS[i]);
        if (i % 2 === parity) {
          digit *= 2;
          if (digit > 9) {
            digit -= 9;
    ...
    key: "getBankPay",
    value: function getBankPay(ccn) {
      var ccnS = Array.from(ccn.replace(/ /g, '').toString());
      if (ccnS.length == 1) {
        if (ccnS[0] == '4') {
          return '.icon2';
        } else if (ccnS[0] == '5') {
    ...
    key: "redrawBank",
    value: function redrawBank() {
      var icons = Array.from(this.parentEl.querySelectorAll('.icon'));
      for (var _i = 0, _icons = icons; _i < _icons.length; _i++) {
        var icon = _icons[_i];
        icon.classList.add('disabled');
      }
      var typeBank = this.getBankPay(this.input.value);
      if (typeBank === null) {
        return;
      }
      var iconBank = this.parentEl.querySelector(typeBank);
